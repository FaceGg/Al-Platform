...
**日期：** 2026-09-01
**最近修订：** 2026-09-25
**状态：** 需求与关键决策已确认，评审通过（2026-09-02）；CLU-02 容量范围于 2026-09-25 修订
**范围：** 通用结构化数据的版本管理、自动建模、手动/自动标注、独立标注员门户、模型资产化与离线导出
**不包含：** 开发排期、任务拆分、代码实现步骤

> **当前容量范围修订（2026-09-25）：** CLU-02 的当前必需容量为 10,000 样本；此前评审材料中的百万样本目标保留在历史记录中，不再作为当前发布门槛。
...
| CLU-02 | 10,000 样本聚类 | 最终赋簇覆盖全量；Silhouette 评估模式/样本数/hash 正确记录，前端不加载全量 |
