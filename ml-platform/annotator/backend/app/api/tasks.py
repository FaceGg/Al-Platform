...
async def get_task(task_id: UUID, assignment_id: UUID | None = None, principal: PortalPrincipal = Depends(require_portal_session)):
    try:
...
async def list_samples(task_id: UUID, assignment_id: UUID | None = None, cursor: str | None = None, limit: int = Query(default=50, ge=1, le=200), sample_search: str | None = Query(default=None, min_length=1, max_length=256), label_status: str | None = Query(default=None, pattern="^(complete|incomplete)$"), comment_status: str | None = Query(default=None, pattern="^(open|resolved|none)$"), modified_after: datetime | None = Query(default=None), authorized_field: str | None = Query(default=None, min_length=1, max_length=128), authorized_value: str | None = Query(default=None, max_length=256), principal: PortalPrincipal = Depends(require_portal_session)):
    try:
        return await PlatformClient().internal_request("GET", f"/api/internal/portal/tasks/{task_id}/samples", subject_id=str(principal.subject_id), scope="assignment:read", params={key: value for key, value in {"cursor": cursor, "limit": limit, "sample_search": sample_search, "label_status": label_status, "comment_status": comment_status, "modified_after": modified_after.isoformat() if modified_after else None, "authorized_field": authorized_field, "authorized_value": authorized_value, **({"assignment_id": str(assignment_id)} if assignment_id else {})}.items() if value is not None})
    except PlatformClientError as error:
        raise _failure(error) from error

@router.put("/tasks/{task_id}/samples/{sample_id}/labels")
async def save_labels(task_id: UUID, sample_id: str, data: LabelWrite, assignment_id: UUID | None = None, principal: PortalPrincipal = Depends(require_portal_session)):
    try:
...
async def bulk_labels(task_id: UUID, data: BulkLabelWrite, assignment_id: UUID | None = None, principal: PortalPrincipal = Depends(require_portal_session)):
    try:
...
async def confirm(task_id: UUID, data: ConfirmationRequest, assignment_id: UUID | None = None, principal: PortalPrincipal = Depends(require_portal_session)):
    try:
...
async def edit_for_return(task_id: UUID, data: ConfirmationRequest, assignment_id: UUID | None = None, principal: PortalPrincipal = Depends(require_portal_session)):
    try:
...
async def return_task(task_id: UUID, data: ConfirmationRequest, assignment_id: UUID | None = None, idempotency_key: str | None = Header(default=None, alias="Idempotency-Key"), principal: PortalPrincipal = Depends(require_portal_session)):
