...
> 基线日期：2026-07-14。状态依据源码、运行时注册表、自动化测试和构建结果，不以文件存在作为完成功能的依据。
...
| API 路由模块 | 24 | `backend/app/api/*.py` 文件统计 |
| FastAPI 路由 | 146 | 导入 `app.main` 后读取 `app.routes` |
| API 端点声明 | 141 | 路由装饰器静态统计，不含框架内建路由 |
| SQLAlchemy 持久化模型 | 30 | `backend/app/models/*.py` 中继承 `Base` 的类 |
| 注册算子 | 80 | 导入 `app.main` 后读取 `OperatorRegistry.list_all()`，80 个 ID 全部唯一 |
| 后端测试文件 | 32 | `backend/tests/test_*.py`；标准套件执行 31 个隔离模块 |
| 前端测试文件 | 9 | Vitest 共 26 个测试 |
...
| DAG 执行与状态推送 | 功能可用 | 严格算子协议、重试、等待超时、协作取消、尝试历史及 WebSocket 测试通过 | 进程重启恢复、断点续跑、进程级硬终止未实现 |
| DataBus | 功能可用 | DataFrame/JSON/二进制相关引擎测试通过 | 大数据流式传输、制品 URI 未实现 |
...
| 四套焊接工业模板 | 可交付 | 真实 Fault 数据四模板后端 E2E、Artifact 向导和浏览器主流程通过 | Ubuntu CI 真实运行待确认 |
| Windows 本地部署脚本 | 可交付 | 启动、健康、登录、停止和端口释放通过 | 服务化、自动升级未实现 |
| Ubuntu 本地部署脚本 | 受阻 | Bash 语法通过，CI 配置已建立 | 分支未推送，无真实 Ubuntu 运行证据 |
...
| Docker Compose 部署 | 受阻 | Compose 文件和 Dockerfile 存在 | 当前机器无 Docker，镜像构建未验证 |
| PostgreSQL/Redis/Celery/MinIO | 未实现 | 依赖声明存在 | 生产配置、迁移和运行闭环缺失 |
| Kubernetes/Notebook/GPU 调度 | 未实现 | 计划中定义 | 无生产实现与验收用例 |

## 本次验证

- 后端：`python run_suite.py`，31/31 模块通过。
- 前端：`npm test`，9/9 文件、26/26 测试通过；存在 jsdom/React `act` 环境告警。
- 前端：`npm run build`，TypeScript 与 Vite 构建通过。
- 浏览器：`npx playwright test --project=chromium`，焊接主流程 1/1 通过并重复验证。
- 构建告警：主 JavaScript 包 2,621.11 kB，需在后续性能阶段按路由拆包。
- Docker：当前机器找不到 `docker` 命令，未完成配置及镜像验证。
