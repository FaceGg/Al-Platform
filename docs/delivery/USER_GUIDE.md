...
1. 进入项目详情的“治理/通知”区域。owner 和 editor 可以管理通知端点与订阅；operator 和 viewer 只能读取项目范围内允许查看的通知，不能修改端点、收件人或订阅。
2. 在端点管理中选择一种通道：**站内通知**、**企业微信**、**邮件**或**通用 Webhook**。端点编辑默认只允许改名称；要更换凭据或目标，必须显式开启“重新配置”，表单不会回显已保存的密钥、邮箱密码、企业微信 Key 或 Webhook 签名密钥。
3. 创建订阅时选择事件类型、严重级别和端点。站内通知的收件人只能从当前项目的受权成员或角色中选择；被移除的成员不会继续接收后续消息。
4. 在顶部通知中心查看自己的未读站内通知并标记已读。外部通道只显示安全投递结果与稳定错误码，不显示服务商原始响应或凭据。
5. 企业微信、邮件和通用 Webhook 端点须由管理员提供受控目标。Webhook 只接受通过 HTTPS、目标地址、重定向、端口、负载大小和超时限制校验的地址；不要配置内网、回环、链路本地或云元数据地址。

通知投递由生产 Celery Worker 和 Beat 异步完成；不要通过浏览器前端保存或复制任何端点凭据。部署配置、就绪检查和受控四通道验收命令见 `PRODUCTION_INFRASTRUCTURE.md`。
