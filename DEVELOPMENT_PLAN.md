...
### 2026-07-30：平台名称统一为“智擎”

- 当前周次：第 9 至第 12 周并行产品支持工作。
- 任务状态：已完成。浏览器标题、国际化平台标题、应用侧栏、登录/注册页、仪表盘中英文副标题及核心导航 E2E 断言已统一使用“智擎”。
- 问题现象：用户可见品牌分散为 `ML 算法平台`、`AI模型训练编排平台`、`Precision Forge` 和多个英文名称，无法形成统一的平台身份。
- 已验证根因：品牌名称以字面量分散在 HTML、国际化字典和页面组件中，且现有回归没有统一约束这些入口。
- 解决方法：新增 `branding.test.ts` 品牌合同后先观察 7/7 预期失败，再精准替换用户可见品牌文案；保留 `ml-platform`、API 路径、Celery 任务名、存储桶、模型来源类型和代码符号等技术标识不变。新增测试同时登记到第 1 周前端验收清单。
- 验证方式：品牌合同由 7/7 RED 转为 7/7 GREEN；品牌与验收清单聚焦回归 9/9 通过；前端全量 Vitest 32 文件、117 用例通过；`npm run build` 通过。生产前端旧品牌扫描无匹配，构建仅保留既有 ECharts 大 chunk 告警。
- 影响范围：仅前端用户可见品牌文案、对应测试和设计注释；未修改后端、API、持久化、认证、工作流执行、ReactFlow 合同或用户已有的 `platform-preview.html` 修改。
- 预防措施：品牌调整必须以明确的用户展示入口清单约束，禁止对 `platform` 做全仓库替换；新增前端测试文件后必须同步更新 `weekAcceptance.test.ts` 的周次清单。
- 遗留事项：本次未运行需要真实账户和服务端生命周期的 Playwright E2E；核心导航断言已更新，组件合同、全量 Vitest 和生产构建已覆盖本次静态品牌变更。

### 2026-07-30：主页侧栏“智擎”标题对比度修正

- 当前周次：第 9 至第 12 周并行产品支持工作。
- 任务状态：已完成。登录后主页左上角“智擎”在浅色主题下改为纯黑 `#000000`，深色主题继续使用浅色 `#E6EDF3`。
- 问题现象：侧栏标题颜色硬编码为浅灰 `#E6EDF3`，在浅色主题的 `#F7FBFC` 侧栏上对比不足。
- 已验证根因：标题颜色没有随主题变化，原颜色只适合深色侧栏。
- 解决方法：根据 `theme` 为标题选择浅色主题纯黑和深色主题浅色，并新增浅色主题渲染回归；测试先因实际颜色为 `rgb(230, 237, 243)` 失败，实施后恢复通过。
- 验证方式：`AppLayout.test.tsx` 聚焦 4/4 通过；前端全量 Vitest 32 文件、118 用例通过；`npm run build` 通过，仅保留既有 ECharts 大 chunk 告警。
- 影响范围：仅 `AppLayout` 侧栏品牌标题及其组件测试；未修改副标题、顶部栏、登录页、后端、认证或工作流行为。
- 预防措施：应用壳中的前景色不得硬编码为只适配单一主题的值；主题相关视觉修复至少覆盖目标主题，并保留另一主题的对比度。
- 遗留事项：未创建或伪造受保护路由的认证状态；浅色模式颜色由组件渲染回归和生产构建覆盖。
