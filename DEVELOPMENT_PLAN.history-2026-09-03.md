...
> 归档日期：2026-09-03
> 来源：DEVELOPMENT_PLAN.md 在本次当前计划整理前的完整快照。
> 用途：保留已完成工作、历史失败/阻断、已被后续结论覆盖的状态及其原始验证记录；当前未完成任务以 DEVELOPMENT_PLAN.md 为准。
> 保留规则：本文件不改写历史结论。若历史条目与较晚的状态同步冲突，以较晚的同步记录和当前计划的明确状态为准。
...
### 当前产品开发计划边界（2026-09-02）

- 顶部 Week 1-12 状态是历史验收冻结记录；不代表新的通用自动建模与数据标注平台已经完成。
- 当前产品开发主计划是 `ml-platform/docs/superpowers/plans/2026-09-02-general-automl-annotation-platform.md`，状态为 `planned`，从 Task 1 开始执行。
- 新平台验收必须使用实现完成后的当前 SHA 和新生成的迁移、导出、离线推理、恢复及浏览器证据；不得复用 Week 9-12 历史证据关闭新计划。
- 旧计划 `ml-platform/docs/superpowers/plans/2026-08-31-multilabel-annotation-annotator-portal.md` 仅用于历史追溯，已被新计划取代。
...
## 96. 技术方案评审通过后的文档一致性修复（2026-09-02）

- 问题现象：通用平台技术方案已完成评审确认，但主方案仍写“评审修订中”；实施计划的验收编号漏列 `DAT-03` 和 `LAB-03`；Task 14 的最终命令未覆盖导出、离线推理、安全、数据库迁移回归和恢复演练；旧实施计划没有明确禁止执行；总计划顶部的历史 Week 1-12 完成状态容易与新平台计划混淆。
- 根因：评审决策、实施计划和历史开发台账分别追加，缺少一次跨文档状态、验收编号、命令和引用的同步审计。
- 修复：将主方案状态更新为 2026-09-02 评审通过；统一 Task 14 验收编号为 `DAT-01..03`、`LAB-01..03`；补充数据库迁移、导出、离线推理、安全、备份恢复和工具回归命令；为旧计划增加历史归档/禁止执行说明；在本节前增加当前产品计划边界；创建通用化迁移清单、通用平台验收矩阵和导出运行时清单三个计划交付物。
- 验证：检查 14 个任务均包含 Step 1-4 且依赖完整；主方案与计划的验收编号一致；计划引用文档均存在；Task 14 恢复命令从 `ml-platform/frontend` 返回 `ml-platform/backend` 后调用仓库已有的 `tools/acceptance/run_backup_restore.sh`，并在同一目录运行 `tests/test_week11_12_tools.py`；Markdown 围栏成对、计划正文无 TODO/TBD 占位词，`git diff --check` 通过。验收矩阵另行逐项列出命令和证据路径。
- 预防措施：评审通过后必须执行一次跨文档合同审计；验收编号以主方案为唯一来源；计划中的每条命令必须对应仓库现有脚本或同时列入创建文件；历史计划必须显式标记为不可执行。

## 97. 文档修复续行（2026-09-03，验收命令与证据上下文）

- 复核范围：继续损坏线程中的文档修复，不恢复旧线程历史，不 fork 旧线程；保留当前工作树全部未提交改动。
- 新发现：Task 14 原命令在宿主机从前端目录返回后端目录直接调用 `run_backup_restore.sh`，没有说明该脚本依赖后端容器、隔离 Compose 环境、数据库/MinIO 凭据和 `ML_PLATFORM_EVIDENCE_DIR`；验收矩阵此前只按范围汇总编号，没有逐项命令、证据来源或回执生成责任。
- 修复：Task 14 改为调用仓库已有的版本化 `ml-platform/backend/tools/acceptance/run_week11_acceptance.sh` 编排恢复演练；矩阵逐项列出主方案第 16.4 节的 19 个验收编号、精确测试命令和证据路径；计划新增 `tools/generic_acceptance_evidence.py`，统一生成脱敏、当前 SHA 绑定的逐项 JSON 回执。
- 验证：`git status --short --branch`、`git diff --stat`、`git diff --check` 已执行；主方案与矩阵编号集合一致（19 项）；计划包含 Task 1-14、每项 Step 1-4 和 14 条依赖；活动文档代码围栏成对，计划/方案/验收/迁移文档无 `TODO` 或 `TBD`；恢复脚本和 `test_week11_12_tools.py` 路径存在。未运行业务测试、迁移、构建、浏览器或真实 Compose 验收，因为本轮只修复文档，且通用平台功能尚未实现。
- 未完成与门禁：通用化迁移清单、验收矩阵、导出运行时清单和通用实施计划仍为 `planned`；新增回执工具、业务实现、测试、真实证据、提交和推送均待后续任务完成，不得把本轮文档校验当作平台功能完成。

## 98. 文档修复续行（2026-09-03，清理证据来源与恢复前置条件）

- 新发现：现有版本化 `run_week11_acceptance.sh` 只调用 `run_backup_restore.sh`、`run_upgrade_fixture.sh` 和性能执行器；仓库中没有 `cleanup.json` 的生成者，Compose teardown 也不能证明临时制品按 TTL 清理。
- 新发现：runner 使用 `--no-build`，且根 Compose 文件对 PostgreSQL、MinIO、Redis、MLflow、TensorBoard、推理运行时、通知密钥和 acceptance 证书有显式环境契约；仅写脚本路径和 `ML_PLATFORM_EVIDENCE_DIR` 会让文档命令在缺少服务/镜像/凭据时产生误导。
- 修复：验收矩阵新增恢复执行上下文、CI 权威 job、Compose 文件/镜像/变量/证书前置条件；明确 backup/upgrade 由版本化 runner 生成，`recovery/cleanup.json` 必须由 Task 13 cleanup harness 生成，缺失时 REL-01 不能通过。Task 13/14 同步写入 cleanup report 字段、来源 SHA 和 fail-closed 规则；迁移清单明确当前表格只是模板占位。
- 历史关系：本节对第 96 节中“直接调用 `run_backup_restore.sh`”的解释作追加覆盖；第 96 节原文保留用于历史追溯，第 97 节（并由本节进一步细化）的 runner/context 说明 supersede 第 96 节，作为当前文档依据。
- 验证与未完成：本轮仍只进行文档和路径合同修复；业务测试、迁移、构建、浏览器、导出、真实 Compose 恢复、cleanup harness、回执工具、提交和推送均未执行，状态继续为 `planned`，不得把文档审计当作功能验收通过。

## 99. 文档修复续行（2026-09-03，Windows 工作树行尾与 WSL runner）

- 新发现：当前工作树的三个版本化验收 shell 文件由 Git `core.autocrlf=true` 物化为 CRLF；在 WSL 中直接执行 `bash -n` 分别在 `cleanup() {`、`elif` 和文件结尾处报语法错误。该现象不是本轮文档补丁写入的内容。
- 根因：`HEAD` 中的 shell blob 是 LF，但 Windows 工作树转换为 CRLF；WSL shell 不会自动忽略这些回车字符。
- 验证：对当前工作树文件的直接 WSL 语法检查为失败；通过 `git cat-file blob HEAD:<path> | wsl.exe -e bash -n` 检查同一版本化内容均返回 `exit=0`。未对现有 shell 文件做行尾转换或覆盖。
- 处置：验收矩阵新增 LF 行尾前置条件，明确 Linux/CI checkout 或独立 LF 校验副本才可作为 runner 语法/运行验证边界；当前 CRLF 工作树不把直接 WSL runner 执行计为通过。
- 未完成与门禁：本轮没有真实 Compose、恢复、清理 harness、业务测试、迁移、构建、浏览器、导出、提交或推送；行尾兼容仍是当前本地执行风险，状态继续为 `planned`。

## 100. 开发环境迁移记录（2026-09-03）

- 迁移来源：`C:\\Users\\17723\\.codex\\worktrees\\d576\\agent_spot_welding`；迁移目标：`E:\\codex_workspace\\agent_spot_welding\\.worktrees\\general-automl-annotation-20260902`。
- 目标工作树已在项目目录内创建并切换到用户指定分支 `general-automl-annotation-20260902`；基线提交为 `8f51a0d9971355c6905fdf3ae8cd22ad957bb3a8`。
- 迁移方式：Git 原生 `worktree move` 因 C: 到 E: 跨卷返回 `Permission denied`；随后在项目内以现有 `codex/general-automl-annotation-20260902` 分支建立本地工作树，仅复制源树的 7 个修改/未跟踪文档路径，未复制 `.git` 元数据。
- 验证：源/目标均为 556 个文件，逐文件 SHA-256 清单 `0` 个不匹配；Git 状态 6 行完全一致；主方案与验收矩阵 19 个编号一致；实施计划包含 Task 1-14、每项 Step 1-4 和 14 条依赖；活动文档无 `TODO/TBD`，`git diff --check` 通过。
- 当前状态：环境迁移已验证，外部源工作树待删除；通用自动建模与数据标注平台仍按主计划保持 `planned`，业务实现、运行时测试、迁移、构建、浏览器、导出、恢复和远程门禁均未因本次环境准备而标记完成。
- 预防措施：以后仅在当前项目目录下创建工作树；跨卷迁移先创建项目内链接工作树，再进行排除 `.git` 的逐文件复制和内容指纹校验，禁止仅凭目录存在或命令退出码认定迁移完成。
