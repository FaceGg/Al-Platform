...
### 2026-08-17：Week 11 工作流性能终态证据收口

- 当前周次：第 11 周固定资源性能验收。
- 任务状态：性能工具与 manifest 合同修复完成，真实 4 vCPU/8 GiB 三轮负载尚待执行。
- 问题现象：原 `welding-e2e` 场景只把 10 次工作流启动 HTTP 请求视为成功，未读取 `run_id`、未轮询 `/api/runs/{run_id}`，因此即使后台全部失败或停留在非终态，性能汇总仍可能通过。
- 已确认根因：通用 HTTP 负载器只保留状态码和延迟，设计文档中的“10/10 completed”没有转化为原始证据字段和 fail-closed gate。
- 解决方法：新增工作流专用负载器，逐次提交后轮询到 `completed`、`failed`、`cancelled` 或超时；证据记录终态计数、10 个完成耗时和最大单次耗时，汇总要求 `completed_requests == requests`、终态只能为 10 个 `completed` 且样本数量一致。
- 验证方式：缺失终态证据的旧格式先被证明会错误通过，新增 gate 后稳定失败；真实本地 HTTP fixture 的 10 次提交与轮询先因 runner 不存在 RED，最小实现后 10/10 GREEN；性能与 manifest 组合 46/46 通过。
- 影响范围：仅 Week 11 性能工具、证据校验与测试夹具；不修改工作流生产 API、调度器、数据库 Schema 或业务执行逻辑。
- 预防措施：异步任务性能验收必须同时记录提交状态和业务终态，不能用 HTTP 2xx 或 task ID 代替完成证据；最终 manifest 的正向夹具必须包含与书面验收条件等价的语义字段。
- 遗留事项：在冻结提交的真实 Compose 栈内运行 10/10 工作流，并将 worker 暂停后的独立队列提交场景与 worker 运行时的完整工作流场景分开测量。
...
### 2026-08-18：工作台、主题、用户信息与数据标注导航回归修复

- 当前状态：四项前端回归已完成实现、聚焦回归、完整 Vitest 和真实浏览器验证；生产构建仍被当前 `main` 既有的 AutoML TypeScript 合同冲突阻塞，不能标记完整构建通过。
- 问题现象：工作台接口失败时静默显示全零；明亮模式顶部栏仍使用暗色硬编码背景；右上角会用 UUID 回退显示用户名且用户名与身份间距过大；左侧导航缺少独立的数据标注入口。
- 已确认根因：PR #21 合并时把 `7af699d` 的数据标注菜单和 `ffd99fd` 的 `/auth/me`、UUID 抑制及主题 Header 修复覆盖回旧实现；Dashboard 又对聚合接口逐项吞错，把失败响应当作空数据渲染。
...
### 2026-08-18：Week 1-12 当前未完成任务清单（有效口径）

- 口径说明：本节是当前未完成工作的权威清单；此前各周的历史记录、失败快照和“进行中”表述全部保留，用于追溯，不再直接代表当前状态。后续状态更新必须以“功能实现、真实运行态验收、远程发布门禁”分栏记录。
- Week 1-5：交付已完成，无当前发布阻塞项。
- Week 6：当前无已确认的发布阻塞项；若重新启用历史兼容性或部署专项，需另建任务并补充验收证据，不得从历史遗留条目推断为当前未完成。
- Week 7：交付已完成，无当前发布阻塞项。
- Week 8：交付已完成，无当前发布阻塞项。
- Week 9：功能实现已完成，不需要重复修复。尚未完成的是最终验收：基于当前冻结 HEAD 重建生产镜像并重新生成 provenance/security receipts，重新执行生产推理生命周期证据，并在 GitHub Actions 配额恢复后完成 required checks。
- Week 10：模型卡、权限/审计、四通道通知等功能实现及本地回归已完成，不需要重复开发。尚未完成的是外部验收矩阵：owner/editor/operator/viewer/outsider 角色权限，应用内、企业微信、邮件、通用 Webhook 四个通知通道，以及重试、死信、脱敏、审计记录和拒绝路径。
- Week 11：固定资源（4 vCPU/8 GiB）长负载仍未闭合；必须先诊断 Docker/WSL 同时收到 `SIGTERM` 的宿主生命周期原因，再完成三轮固定资源运行，证明 10/10 工作流业务终态完成，并区分 worker 暂停时的入队测试与 worker 正常运行时的端到端测试。还需生成有效的性能原始证据、`welding-e2e` 和 `performance/summary.json`；完成真实 PostgreSQL/MinIO 备份恢复（表计数、值、外键、对象 SHA-256、RTO/RPO）以及真实 N-1 迁移（`20260718_08` 到冻结 Week 10 head）的升级、一致性、readiness、worker、通知/API smoke 验证。
- Week 12：尚未完成最终发布冻结和安全验收。需要冻结单一源代码提交，重建 backend/worker/inference/TensorBoard 四个当前 HEAD 生产镜像，核对 image ID、OCI revision、非 root smoke 和源代码 provenance；重新执行 Trivy 镜像/文件系统扫描、Gitleaks、Bandit、pip-audit、npm audit 及完整 `tools.security_scans` 链；执行外部 Chromium、角色矩阵和四通道通知矩阵；在目标服务器移除 Nginx UTF-8 BOM、重建 Nginx 与 frontend，并验证 IPv4 health check 和 80 端口；生成 fail-closed evidence manifest 与最终验收报告；在同一冻结提交上完成后端、前端、构建、迁移和浏览器回归。
- Week 12 发布收口：同步 `DEVELOPMENT_PLAN.md` 与 `PLATFORM_STATUS.md`，清理或明确关闭与当前主线不一致的历史文档分支/开放 PR；Actions 配额恢复后分别验证 PR light、main full、manual full 和 cleanup workflow，并记录远端 CI URL、证据、最终 SHA 与残余风险。
- 非阻塞技术债（不计入上述验收完成度）：本地线程执行器无法安全强杀第三方/Python 调用，取消仍为协作式；历史固定 `0.5s` WebSocket 等待仍需在可复现场景下移除或重新验证；登录页可能把网络/422/401 合并成同一错误提示；ECharts 大 chunk 警告和偶发 `ResizeObserver` 警告仍可作为后续优化；AutoML 第三方估计器强制终止及长时 Celery/MLflow 预算验证仍是架构/性能跟进项。
- 外部阻塞：GitHub Actions 月度配额/计费当前耗尽，近期运行在 runner 启动前失败，`steps=[]`，并非代码或测试失败。不得循环 rerun、降低 required checks 或把“未执行”改写为“通过”；计费恢复后必须从同一冻结提交重新执行远端门禁。
- 当前结论：Weeks 1-8 已完成，Week 9/10 实现已完成；当前真正未完成的是 Week 11/12 的真实环境验收、最终安全/发布证据、远程 required checks 及其计费恢复后的复核。历史记录继续保留，不得据此重复开展 Week 9/10 实现工作。

### 2026-08-18：OPTIMIZATION_PLAN 逐项核对、开发进度与工期估算

- 核对范围：已将 `OPTIMIZATION_PLAN.md`（2026-07-23 版本）的 17 项候选能力与当前主线代码、Week 1-12 有效状态和第 13-17 周规划交叉核对。该文档是产品优化候选方案，不是当前 Week 1-12 的完成清单；历史记录中的“已规划”或“可复用”不等于已实现。
- Week 1-12 当前进度：Week 1-8 交付完成；Week 9/10 功能实现、迁移、本地回归和前端修复完成；Week 11/12 的固定资源性能、真实备份恢复、N-1、外部 Chromium/角色/通知矩阵、最终安全与 evidence manifest、目标服务器运行态修复和远程 required checks 仍未闭合。GitHub Actions 配额耗尽是外部门禁阻塞，不计为代码失败。

#### 优化方案逐项状态

| 能力 | 当前状态 | 处理口径 |
|---|---|---|
| MLflow autolog | 未确认落地；当前已有 `MlflowClient`/追踪封装，但不能据此视为 SDK autolog 已接入 | 后续 Week 9 优化候选，需单独实现并补回归 |
| 模型阶段流转 | 未完成；当前主线以审批状态和部署 rollout 为主，没有完整 Staging/Production 阶段合同 | 后续产品能力，不列入 Week 9/10 重复修复 |
| 运行级缓存 | 未完成；DAG 执行器没有方案要求的输入哈希、命中跳过和失效 API | 第 11 周优化候选，需与现有 inference cache 区分 |
| 错误分支与可配置重试 | 部分具备重试，错误分支未完成 | 后续工作流增强 |
| Webhook/Event 触发器 | 未完成；已有 schedule，不等于事件触发 | 后续工作流增强 |
| Kubernetes Job/Pod 执行器 | 未开始 | 第 14 周规划，依赖 Kubernetes 基础接入 |
| JupyterHub/Notebook | 未开始 | 第 15 周规划 |
| GPU 调度与资源配额 | 未开始；现有 GPU 监控/展示不等于 Kubernetes GPU 调度 | 第 15 周规划 |
| 多集群路由与资源治理 | 未开始；当前仍为单机 Compose | 第 16 周规划，先做必要性评估 |
| 行业化算子作业模板 | 未开始；现有算子目录不等于行业模板体系 | 第 13 周规划 |
| Superset SQL Lab/数据探索 | 未开始 | 第 17 周规划 |
| Label Studio 多模态标注 | **延后**；当前保留基础数据标注页面和独立菜单，不建设 Label Studio 引擎、任务 API、iframe 或训练数据回流集成 | 已移出当前承诺范围，后续仅在明确出现多模态标注需求时重新评估 |
| 数据质量报告 | 未开始 | 第 17 周规划 |
| 类型化端口强校验 | 部分具备 `PortSpec.type` 和结果校验，尚未达到方案要求的全链路强制绑定 | 横向增强，需单独定义兼容边界 |
| 动态工作流 | 未开始；当前为静态 DAG | P4 延后 |
| 画布与工作流代码互转 | 未开始；当前只有画布定义 | P4 延后 |
| 可复现训练环境打包 | 未开始 | 第 12 周优化候选，但不属于当前 Week 12 安全/验收门禁 |

- 当前未完成项归类：以上 17 项中，没有一项应被回填为 Week 9/10 的重复实现任务；Label Studio 集成、动态工作流和画布与代码互转已明确延后，其余候选按第 13-17 周或横向增强另行立项。Week 11/12 的最终验收工作仍按上一节清单独立推进。

#### 工期估算

- Week 11/12 验收收口：在固定资源宿主、真实 PostgreSQL/MinIO、可用扫描器和 GitHub Actions 计费恢复后，预计 **8-12 个有效工作日** 完成重跑、证据生成、回归和文档收口；外部环境等待可能使日历周期延长至 **2-4 周**。计费未恢复前只能完成本地准备，不能承诺远程门禁关闭。
- 优化方案完整候选：原方案列出的单项估算合计约 **61 人日**，不含需求澄清、跨服务集成、迁移、浏览器验收、Kubernetes 集群准备和远程 CI。按 2 名工程师并行，合理日历估算为 **6-8 周**；按 1 名工程师顺序实施约 **12-15 周**。
- 推荐首期范围：暂缓 Label Studio、动态工作流和画布↔代码互转，保留其余 14 项候选；对应约 **47 人日**，按 2 名工程师约 **5 周开发 + 1-2 周集成验收**，总计约 **6-7 周**。多集群路由虽保留在候选范围，但应在 Kubernetes、Notebook、GPU 和资源配额完成后再决定是否进入承诺范围。
- 估算不确定性：Kubernetes 集群、GPU 节点、JupyterHub、Superset、外部通知接收器和远程 Actions 都是外部依赖；若这些环境未准备好，以上工期只代表代码开发时间，不能代表可发布时间。

- 状态结论：当前优先级仍是关闭 Week 11/12 最终验收门禁；优化方案应从第 13 周开始按 P0/P1/P2/P3 分阶段立项。后续周报必须同时标注“计划候选”“代码实现”“真实验收”和“远程发布”四种状态，禁止用优化方案中的复用率或预计周期替代完成证据。

### 2026-08-18：Label Studio 集成暂缓决策

- 用户决策：当前阶段暂时不需要 Label Studio 集成，将其从第 17 周和优化方案首期承诺范围移除，状态标记为“延后”，不再作为当前未完成项或发布阻塞项。
- 保留范围：保留现有数据标注页面、`/data-annotation` 菜单和平台自身已经具备的数据管理能力；本次不删除现有页面或路由。
- 暂停范围：不部署 Label Studio 服务，不实现 iframe 嵌入、Label Studio 任务 API、数据集同步、标注结果回流和训练触发闭环。
- 重新评估条件：只有在明确出现图像、视频、文本或音频多模态标注需求，并确认部署、安全、账号、存储和数据回流边界后，才重新立项。
- 工期影响：推荐首期优化范围从 15 项调整为 14 项，估算从约 50 人日调整为约 47 人日；2 名工程师的开发与集成验收预计由 6-8 周调整为约 6-7 周。Week 11/12 最终验收工期不受此决策影响。

### 2026-08-18：自动建模、模型库和数据标注入口调整

- 实现状态：前端已完成三项交互调整。自动建模默认仅显示“通用自动建模”，隐藏点焊质量感知页签及点焊任务来源；通用建模完成结果支持生成 JSON 分析报告并导出。模型库注册模型和推理部署均接入已有 DELETE API，增加权限控制、二次确认和列表状态更新。数据标注移除五类类型选择卡，直接进入点焊标注任务列表，旧 `type/runId/datasetId` 链接继续兼容。
- 验证方式：前端生产构建 `npm run build` 通过；聚焦 Vitest `47 passed, 8 skipped`，跳过项为被需求屏蔽的点焊质量感知旧测试场景；`git diff --check` 通过。
- 剩余工作：若需要后端生成 Office/PDF 格式的通用自动建模报告，应另行定义报告 API 和制品存储合同；当前导出为前端生成的 JSON 报告，不改变后端训练任务协议。

### 2026-08-18：智擎品牌与通用自动建模任务详情页

- 实现状态：左侧产品品牌统一使用 i18n 的“智擎”，中英文界面不再显示 `Precision Forge`。自动建模入口页收敛为任务列表；右上角“新建”打开通用自动建模参数弹窗，提交后跳转 `/automl/task/:taskId`。任务详情页轮询训练状态，显示总体进度、模型结果和 AUC/F1，按 AUC 降序、AUC 相同时按 F1 降序排列。
- 验证方式：`npm run build` 通过；`git diff --check` 通过。聚焦 Vitest 因 `node_modules/.vite-temp` 当前文件权限返回 `EPERM`，本次未取得新测试结果；此前同一前端依赖环境的构建验证正常。
- 剩余工作：应在修复 `node_modules/.vite-temp` 写权限后补跑 AutoML/AppLayout 聚焦回归，并增加 `AutoMLTaskPage` 的轮询、排序和路由跳转测试。

### 2026-08-18：本地实验数据与辅助目录停止 Git 跟踪

- 用户要求：`spot_weld_quality_experiment_1875.csv`、`spot_weld_quality_experiment_60.csv` 以及根目录 `docs/`、`docs2/`、`tmp/`、`temp_test/` 仅保留在本地，不再上传 GitHub。
- 实施方式：根 `.gitignore` 增加精确文件和目录规则；对已经进入仓库历史的四个目录执行仅索引移除，本地文件保持不变。两个 CSV 原本未跟踪，添加规则后不再出现在普通 Git 状态中。
- 验证方式：`git check-ignore -v` 确认六个目标均命中根 `.gitignore`；`git ls-files -- docs docs2 tmp temp_test ...` 无输出；PowerShell `Get-Item` 确认所有目录和文件仍在本地。
- 发布影响：下一次提交这些索引变更后，GitHub 分支会删除原先已跟踪的 `docs/`、`docs2/`、`tmp/` 和 `temp_test/.gitkeep`，但开发机本地副本不会删除。该变更不包含或覆盖现有前端未提交修改。

### 2026-08-18：通用自动建模任务删除、指标与报告修复

- 实现状态：普通自动建模任务删除不再依赖当前项目选择，直接调用现有 `/training/batch-delete` 接口并刷新列表。任务进度页的模型结果统一读取 `models`、`all_results` 或 `algorithm_results`，并支持从嵌套 `metrics`、`evaluation`、`scores` 读取 AUC/F1；标题已调整为“模型结果”。
- 报告能力：任务进度页增加“生成分析报告”和“导出分析报告”按钮，报告包含任务、项目、状态、进度、最佳模型、排序后的模型结果及原始指标，导出为 JSON。
- 验证方式：前端 `npm run build` 通过；聚焦 Vitest `6 passed, 19 skipped`；`git diff --check` 通过。跳过项为已屏蔽的旧点焊质量感知和默认表单测试场景。
- 剩余风险：现有普通 AutoML 后端仅在返回值中存在 AUC/F1 时展示该指标；若需要所有分类算法始终产出 AUC/F1，需单独扩展训练评估协议并为多分类概率不可用模型定义降级规则。

### 2026-08-18：分类 AutoML 真实 AUC/F1 与报告可用条件

- 实现状态：每个算法族完成超参数搜索后，使用其最佳参数模型执行独立的分层交叉验证或固定留出评估，计算真实 ROC-AUC 和 weighted F1；指标写入 `algorithm_results`、`all_results`、`best_model` 及模型库指标。多分类 AUC 使用 weighted one-vs-rest，无法生成概率/决策分数时 AUC 保留为空，不使用 accuracy 伪装。
- 页面状态：任务详情页只有在任务状态为 `completed`、进度达到 100% 且存在模型结果时才启用“生成分析报告”；导出按钮依赖已生成报告。
- 验证方式：后端模块编译通过；使用 `make_classification` 的真实随机森林交叉验证冒烟验证得到有限范围内 AUC/F1；前端生产构建通过；聚焦 Vitest `6 passed, 19 skipped`；`git diff --check` 通过。
- 剩余风险：极小样本或单类别数据无法定义可靠 AUC，页面会显示 `-`；需要在数据集校验层继续提示类别数和每类最小样本量。

### 2026-08-18：模型结果超参数搜索详情

- 实现状态：算法族结果现在携带所有超参数试验的编号、状态、Accuracy、具体参数、耗时和错误码。模型结果表增加“详细”按钮，弹窗展示该模型的 AUC、F1、Best Accuracy、最佳参数及完整试验列表；分类任务界面不再使用含义模糊的“搜索分数”文案。
- 验证方式：前端生产构建通过；后端模块编译通过；`git diff --check` 通过。
- 剩余风险：旧版或非 Optuna 兼容任务没有试验明细时，详情弹窗显示“暂无超参数搜索记录”，但仍显示已有 AUC/F1 和最佳参数。

### 2026-08-18：模型详细结果布局优化

- 实现状态：详细结果弹窗改为“指标摘要、最佳参数、搜索记录”三段式布局；AUC、F1、Best Accuracy 和试验次数集中显示，最佳参数支持滚动查看，搜索记录表支持固定高度滚动和长参数换行。
- 验证方式：前端 `npm run build` 通过。

### 2026-08-18：自动建模改动发布到 GitHub main
...
### 2026-08-22：Week 11/12 最终收口任务清单（未完成，不得提前关闭）

本节是当前可执行的收口清单。远程 full run `32569941915`（冻结 SHA `252abf77c547bdb637755b00d1ff1aec8f13e14d`）已通过 Quality、生产/实验集成、Chromium 和 Week 11-12 verification，但不替代下列真实运行态证据。

#### W11-R1：固定资源真实性能三轮

- 前置条件：隔离 WSL/Docker Compose；资源封装固定为 4 vCPU / 8 GiB；PostgreSQL、Redis、MinIO、MLflow、backend、worker、scheduler、inference-runtime、tensorboard-gateway 全部 healthy；服务在整个负载窗口内不得被宿主同时终止。
- 执行范围：`core-read`、`warm-inference`、`enqueue` 各执行 iteration 1/2/3；`cold-model-load` 执行 iteration 1；`welding-e2e` 执行 iteration 1，要求 10 个工作流请求均进入 `completed`。
- 通过标准：每个原始 JSON 的 load、请求计数、状态计数、错误率、延迟和提交 SHA 通过 `week11_performance.validate_iteration_evidence`；`performance/summary.json` 的 `status` 和 `candidate_status` 均为 `passed`；保留全部原始 JSON、资源封装和容器 image/revision 证据。
- 当前状态：未完成。此前固定资源负载中 backend、Redis、PostgreSQL、worker、scheduler 同时退出，性能汇总为 `failed`。

#### W11-R2：真实 PostgreSQL/MinIO 备份恢复

- 前置条件：真实隔离 PostgreSQL 与 MinIO；源数据库、恢复数据库、源 bucket、恢复 bucket 必须分别命名并显式确认隔离；备份工具使用环境变量传递凭据，证据不得写入 URL 或日志。
- 执行范围：创建 PostgreSQL 备份和 MinIO 对象镜像；恢复到不同数据库和 bucket；验证表计数、关键数据值、外键、对象 SHA-256、备份/恢复 return code、RTO 与 RPO。
- 通过标准：`backup/restore-result.json` 的 `status=passed`、`row_counts_equal=true`、`foreign_key_violations=[]`、对象哈希全部匹配、`rto_passed=true`、`rpo_passed=true`；同时生成完整签名备份 manifest 和两类完成 receipt。
- 当前状态：未完成。尚无真实恢复通过结果。

#### W11-R3：带代表性数据的真实 N-1 升级

- 前置条件：隔离 PostgreSQL 从实际双 head `20260720_10_security_notifications` 与 `20260730_09` 开始；不得使用默认 `ml_platform` 目标；先执行 `upgrade_fixture.py seed` 写入非零 `users`、`projects`、`workflows`、`model_library`。
- 执行范围：生成 N-1 快照；升级到 `20260819_12`；连续执行两次 upgrade；运行 `alembic current`、`alembic check`；比较升级前后行数和代表性数据；执行升级后 readiness、worker、API、通知 smoke。
- 通过标准：`upgrade/result.json` 的 `status=passed`；`first_upgrade=ok`、`second_upgrade=ok`、`alembic_check=ok`；前后快照有效、代表性表数据无损、外键违规为空；证据中记录实际双 head、目标 head 和隔离数据库摘要，不记录凭据。
- 当前状态：工具修复已完成，真实带数据升级未完成。空库结果 `UPGRADE_SNAPSHOT_INVALID` 不得计为通过。

#### W12-R1：最终安全/发布 evidence manifest

- 前置条件：沿用冻结 SHA `252abf77c547bdb637755b00d1ff1aec8f13e14d`；下载并核对 full run artifact、四个生产镜像 image ID/OCI revision/non-root smoke、扫描回执和 Chromium 结果。
- 必须纳入的文件：`environment.json`、`performance/summary.json`、`backup/restore-result.json`、`upgrade/result.json`、`security/summary.json`、`playwright/result.json`、`security/runtime-images.json`。
- 通过标准：`evidence_manifest.py` 对所有必需文件、状态、哈希、提交绑定、扫描 gate、浏览器 gate 和运行态 gate 全部通过；生成最终 manifest 和验收报告；任何 `skipped` 不得改写为 `passed`。
- 当前状态：未完成，因 W11-R1/W11-R2/W11-R3 尚无完整通过证据。

#### W12-R2：发布目标和外部验收矩阵（按当前发布范围执行）

- 执行范围：owner/editor/operator/viewer/outsider 角色矩阵；站内、企业微信、邮件、通用 Webhook 四通道；重试、死信、脱敏、审计和拒绝路径；目标服务器 Nginx BOM 清理、frontend/Nginx 重建、IPv4 health check 和 80 端口检查。
- 通过标准：每个角色/通道/拒绝路径均有原始响应或事件证据，并绑定冻结 SHA；目标服务器检查全部成功；失败路径上传的证据脱敏且不含凭据。
- 当前状态：远程 Chromium 和受控通知栈已通过，但完整外部矩阵和目标服务器证据仍需单独核对；未核对前不得视为完成。

#### W12-R3：最终状态更新与发布决策

- 完成条件：W11-R1、W11-R2、W11-R3、W12-R1、W12-R2 全部通过；更新本节状态、`PLATFORM_STATUS.md`、最终验收报告和远程证据链接；确认工作区无未纳入发布的 Week 12 修改。
- 当前状态：未完成。Week 11/12 总体保持“进行中”。

**执行入口：**验收执行以 [`ml-platform/docs/superpowers/plans/2026-08-22-week9-12-acceptance-closure.md`](ml-platform/docs/superpowers/plans/2026-08-22-week9-12-acceptance-closure.md) 为唯一任务顺序和证据要求来源；本节只记录状态，不再另行创建平行清单。

### 2026-08-23：冻结 SHA 真实运行验收再次启动，外部镜像拉取超时

- 当前 SHA：`252abf77c547bdb637755b00d1ff1aec8f13e14d`；本地业务镜像 `backend`、`worker`、`inference-runtime`、`tensorboard-gateway`、`migrate`、`scheduler`、通知接收器和代理均已成功冷构建。
- WP0 运行结果：WSL Docker Engine `29.7.2`、Compose `5.4.0` 可用；但 Docker Hub 拉取 `postgres:16`/`redis:7`/`minio`/`mlflow`/`mailpit` 时发生 registry 网络超时，Compose 在启动前退出。该结果是外部运行环境阻断，不是业务通过，也不能生成当前 SHA 的性能、备份或 N-1 通过证据。
- 新证据：`temp_test/week11-12-live/evidence/compose-config-redacted.yml`；构建/拉取原始日志来自本次 WSL 命令输出。未启动的服务未被标记为 skipped 或 passed，旧提交 `b8b968c…` 的历史性能和备份文件继续视为 stale evidence。
- 本地容器验证：仓库根目录挂载到容器后，Week 11 工具与合同测试 `104/104 OK`；错误挂载 backend 子目录会导致安全测试根目录解析失败，禁止将该错误结果归因于产品。
- 当前状态：W11-R1、W11-R2、W11-R3、W12-R1、W12-R2、W12-R3 仍未完成。解除条件是恢复外部镜像拉取或提供等价的已审计镜像缓存，然后从当前 SHA 重新执行 WP0，并重新生成所有 SHA 绑定的运行态证据。

### 2026-08-23：按收口计划完成 WP0 可执行部分，运行栈仍受 registry 阻断

- PowerShell 运行规范：本次宿主侧命令统一使用 PowerShell 7.6.4；WSL 内核资源确认是 4 vCPU、7.76 GiB 可见内存（8 GiB envelope）和 2 GiB swap。
